# QLens
This repository contains the code for the developing quantum-inspired interpretability framework: <ins>"QLens: Towards A Quantum Perspective of Transformers."</ins>

**Abstract:** Current methods for understanding Transformers are successful at extracting intermediate output probability distributions during inference. However, these approaches function as limited diagnostic checkpoints, lacking a mathematical framework for modeling the dynamics of how layers facilitate transitions between these distributions. We propose QLens, a novel framework that leverages the pre-built mathematical toolkit of quantum mechanics for modeling the evolution of Transformer probability distributions during inference. Under QLens, these models are studied by converting their latent activations into a state vector in a Hilbert space derived from their output units. The evolution of this state through subsequent hidden layers is analyzed with unitary operators and analogously defined Hamiltonians. To demonstrate QLens' potential, we conduct a proof-of-concept by probing three single-Transformer-block models across multiple task domains to investigate the influence of individual layers in prediction trajectories. This analysis reveals that probability transformations executed by layers tend to exhibit intra-layer cohesion, inter-layer correlation, systematic bias, and partially concept-driven structure. We present our work as a foundation for physics-inspired methods to be leveraged towards a broader understanding of Transformers.
